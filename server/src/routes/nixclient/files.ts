...
export default async function fileRoutes(fastify: FastifyInstance) {
  ...
  // GET /api/nixclient/files?path=
  fastify.get('/', { preHandler }, async (request, reply) => {
    const user = request.user!
    if (user.role !== 'user') return reply.code(403).send({ success: false, error: 'NixClient users only' })
  ...
  // GET /api/nixclient/files/content?path=
  ...
  // PUT /api/nixclient/files/content — save file content
  ...
  // POST /api/nixclient/files/mkdir
  ...
  // POST /api/nixclient/files/rename
  ...
  // POST /api/nixclient/files/chmod
  ...
  // POST /api/nixclient/files/compress — zip files

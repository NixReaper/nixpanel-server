...
export default function FTP() {
  ...
  const fetch = async () => {
    setLoading(true)
    try { const r = await api.get('/cpanel/ftp'); setAccounts(r.data.data) }
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try { await api.post('/cpanel/ftp', form); setShowModal(false); fetch() }
  ...
  const del = async (id: number) => {
    ...
    try { await api.delete(`/cpanel/ftp/${id}`); fetch() } catch {}
  }

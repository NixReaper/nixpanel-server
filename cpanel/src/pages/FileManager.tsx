...
export default function FileManager() {
  ...
  const loadDir = async (p: string) => {
    ...
    try {
      const r = await api.get(`/cpanel/files?path=${encodeURIComponent(p)}`)
  ...
  const openFile = async (filePath: string) => {
    try {
      const r = await api.get(`/cpanel/files/content?path=${encodeURIComponent(filePath)}`)
  ...
  const saveFile = async () => {
    ...
    setSaving(true)
    try {
      await api.put('/cpanel/files/content', { path: editing.path, content: editing.content })
      setEditing(null)
    } catch (e: unknown) {
  ...
  const deleteSelected = async () => {
    ...
    try {
      await api.delete('/cpanel/files', { data: { paths: Array.from(selected) } })
      loadDir(path)
    } catch {}
  }
  ...
  const createDir = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      await api.post('/cpanel/files/mkdir', { path: `${path}/${newDir}` })

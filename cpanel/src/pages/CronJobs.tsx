...
export default function CronJobs() {
  ...
  const fetch = async () => {
    setLoading(true)
    try { const r = await api.get('/cpanel/cron'); setJobs(r.data.data) }
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try { await api.post('/cpanel/cron', form); setShowModal(false); setForm({ schedule: '0 * * * *', command: '', comment: '', enabled: true }); fetch() }
  ...
  const del = async (id: number) => {
    ...
    try { await api.delete(`/cpanel/cron/${id}`); fetch() } catch {}
  }

  const toggle = async (job: CronJob) => {
    try { await api.put(`/cpanel/cron/${job.id}`, { enabled: !job.enabled }); fetch() } catch {}
  }

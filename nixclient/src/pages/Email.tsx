...
export default function Email() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const [em, fw] = await Promise.all([api.get('/nixclient/email'), api.get('/nixclient/email/forwarders')])
  ...
  const createAccount = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/nixclient/email', form)
  ...
  const del = async (id: number) => {
    ...
    try { await api.delete(`/nixclient/email/${id}`); fetch() } catch {}
  }

  const addForwarder = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true); setError('')
    try { await api.post('/nixclient/email/forwarders', fwdForm); setShowFwd(false); fetch() }
    catch (err: unknown) { setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed') }
    finally { setSaving(false) }
  }

  const delFwd = async (id: number) => {
    try { await api.delete(`/nixclient/email/forwarders/${id}`); fetch() } catch {}
  }

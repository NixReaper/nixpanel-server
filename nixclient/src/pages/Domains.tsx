...
export default function Domains() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const r = await api.get(`/nixclient/domains/${tab}`)
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === 'subdomains') await api.post('/nixclient/domains/subdomains', { subdomain: form.subdomain })
      else if (tab === 'addon') await api.post('/nixclient/domains/addon', { domain: form.domain })
      else await api.post('/nixclient/domains/parked', { domain: form.domain })
  ...
  const del = async (id: number) => {
    ...
    try {
      if (tab === 'subdomains') await api.delete(`/nixclient/domains/subdomains/${id}`)
      else if (tab === 'addon') await api.delete(`/nixclient/domains/addon/${id}`)
      else await api.delete(`/nixclient/domains/parked/${id}`)
      fetch()
    } catch {}
  }

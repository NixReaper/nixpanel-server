...
export default function Databases() {
  ...
  const fetch = async () => {
    setLoading(true)
    try { const r = await api.get('/nixclient/databases'); setDbs(r.data.data) }
  ...
  const createDb = async (e: React.FormEvent) => {
    ...
    try { await api.post('/nixclient/databases', dbForm); setShowDbModal(false); setDbForm({ name: '' }); fetch() }
  ...
  const deleteDb = async (id: number) => {
    ...
    try { await api.delete(`/nixclient/databases/${id}`); fetch() }
  ...
  const createUser = async (e: React.FormEvent) => {
    ...
    try { await api.post(`/nixclient/databases/${showUserModal}/users`, userForm); setShowUserModal(null); setUserForm({ username: '', password: '', privileges: 'ALL' }); fetch() }
    catch (err: unknown) { setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed') }
    finally { setSaving(false) }
  }

  const deleteUser = async (dbId: number, userId: number) => {
    try { await api.delete(`/nixclient/databases/${dbId}/users/${userId}`); fetch() }
    catch {}
  }

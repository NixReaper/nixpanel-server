...
export default function Email() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const [em, acc] = await Promise.all([api.get('/nixserver/email'), api.get('/nixserver/accounts?limit=100')])
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/nixserver/email', { ...form, accountId: parseInt(form.accountId, 10), quotaMb: form.quotaMb })
  ...
  const del = async (id: number, address: string) => {
    ...
    try { await api.delete(`/nixserver/email/${id}`); fetch() }

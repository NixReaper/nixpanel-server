...
export default function Packages() {
  ...
  const fetch = async () => {
    setLoading(true)
    try { const r = await api.get('/nixserver/packages'); setPackages(r.data.data) }
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) await api.put(`/nixserver/packages/${editing.id}`, form)
      else await api.post('/nixserver/packages', form)
  ...
  const del = async (p: Package) => {
    ...
    try { await api.delete(`/nixserver/packages/${p.id}`); fetch() }
    catch (err: unknown) {

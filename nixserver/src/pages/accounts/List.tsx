...
import { Plus, Search, MoreVertical, UserX, UserCheck, Trash2, RefreshCw, ChevronLeft } from 'lucide-react'
...
export default function ListAccounts() {
  ...
  const handleAction = async (id: number, act: 'suspend' | 'unsuspend' | 'terminate') => {
    setActionMenu(null)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-white">List Accounts</span>
      </div>

      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-white text-xl font-semibold">List Accounts</h1>
          <p className="text-[#64748b] text-sm">{data?.total ?? 0} total accounts</p>
        </div>
        <Link
          to="/accounts/create"
                ...
                <td className="px-4 py-3 relative">
                  ...
                  {actionMenu === acc.id && (
                    <div className="absolute right-4 top-8 z-10 bg-[#1e2130] border border-[#2a2d3e] rounded-lg shadow-lg py-1 w-36">
                      {acc.status === 'active' ? (
                        <button onClick={() => handleAction(acc.id, 'suspend')} className="flex items-center gap-2 w-full px-3 py-2 text-amber-400 hover:bg-[#2a2d3e] text-xs">
                          <UserX size={14} /> Suspend
                        </button>
                      ) : acc.status === 'suspended' ? (
                        <button onClick={() => handleAction(acc.id, 'unsuspend')} className="flex items-center gap-2 w-full px-3 py-2 text-emerald-400 hover:bg-[#2a2d3e] text-xs">
                          <UserCheck size={14} /> Unsuspend
                        </button>
                      ) : null}
                      <button onClick={() => handleAction(acc.id, 'terminate')} className="flex items-center gap-2 w-full px-3 py-2 text-red-400 hover:bg-[#2a2d3e] text-xs">

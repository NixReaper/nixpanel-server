...
export default function Resellers() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/nixserver/resellers${params}`)
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try { await api.post('/nixserver/resellers', form); setShowModal(false); fetch() }
  ...
  const act = async (id: number, action: 'suspend' | 'unsuspend' | 'delete') => {
    ...
    try {
      if (action === 'suspend') await api.post(`/nixserver/resellers/${id}/suspend`)
      else if (action === 'unsuspend') await api.post(`/nixserver/resellers/${id}/unsuspend`)
      else await api.delete(`/nixserver/resellers/${id}`)
      fetch()

...
export default function Backup() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const [b, a] = await Promise.all([api.get('/nixserver/backup/list'), api.get('/nixserver/accounts?limit=100')])
  ...
  const backupAccount = async () => {
    ...
    setBacking(true)
    try { await api.post(`/nixserver/backup/account/${selectedAccount}`); fetch() }
  ...
  const backupServer = async () => {
    ...
    setBacking(true)
    try { await api.post('/nixserver/backup/server'); fetch() }
    catch (err: unknown) { alert((err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Backup failed') }
    finally { setBacking(false) }
  }
  ...
  const del = async (path: string) => {
    ...
    try { await api.delete('/nixserver/backup', { data: { path } }); fetch() }
    catch {}
  }

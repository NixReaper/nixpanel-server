...
export default function SSL() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const [sl, acc] = await Promise.all([api.get('/nixserver/ssl'), api.get('/nixserver/accounts?limit=100')])
  ...
  const issue = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/nixserver/ssl/issue', { ...form, accountId: parseInt(form.accountId, 10) })
  ...
  const revoke = async (id: number) => {
    ...
    try { await api.post(`/nixserver/ssl/${id}/revoke`); fetch() }

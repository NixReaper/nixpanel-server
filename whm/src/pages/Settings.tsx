...
export default function Settings() {
  ...
  const fetch = async () => {
    setLoading(true)
    try { const r = await api.get('/whm/system/settings'); setSettings(r.data.data) }
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try { await api.put('/whm/system/settings', settings); setSaved(true); setTimeout(() => setSaved(false), 2000) }

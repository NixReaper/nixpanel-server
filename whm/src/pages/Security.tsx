...
export default function Security() {
  ...
  const fetch = async () => {
    ...
    try {
      const [b, e] = await Promise.all([
        api.get('/whm/security/blocked-ips'),
        api.get('/whm/security/events'),
      ])
  ...
  const block = async (e: React.FormEvent) => {
    e.preventDefault()
    try { await api.post('/whm/security/blocked-ips', { ip: newIp, reason }); setNewIp(''); setReason(''); fetch() }
    catch (err: unknown) { alert((err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed') }
  }

  const unblock = async (id: number) => {
    try { await api.delete(`/whm/security/blocked-ips/${id}`); fetch() }
    catch (err: unknown) { alert((err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed') }
  }

  const resolve = async (id: number) => {
    try { await api.post(`/whm/security/events/${id}/resolve`); fetch() }
    catch {}
  }

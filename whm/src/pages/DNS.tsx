...
export default function DNS() {
  ...
  const fetchZones = async () => {
    setLoading(true)
    try { const r = await api.get('/whm/dns'); setZones(r.data.data) }
    finally { setLoading(false) }
  }

  const loadZone = async (zone: Zone) => {
    setSelected(zone)
    const r = await api.get(`/whm/dns/${zone.id}`)
  ...
  const addRecord = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.post(`/whm/dns/${selected.id}/records`, { ...form, priority: form.priority ? parseInt(form.priority, 10) : undefined })
      setShowForm(false)
      loadZone(selected)
  ...
  const deleteRecord = async (recordId: number) => {
    ...
    try { await api.delete(`/whm/dns/${selected.id}/records/${recordId}`); loadZone(selected) }
    catch {}
  }

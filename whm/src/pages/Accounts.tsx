...
export default function Accounts() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/whm/accounts?${params}`)
      setData(r.data.data)
    } finally {
      setLoading(false)
    }
  ...
  const action = async (id: number, act: 'suspend' | 'unsuspend' | 'terminate') => {
    ...
    try {
      if (act === 'suspend') await api.post(`/whm/accounts/${id}/suspend`)
      else if (act === 'unsuspend') await api.post(`/whm/accounts/${id}/unsuspend`)
      else await api.delete(`/whm/accounts/${id}`)
      fetch()
    } catch (e: unknown) {
